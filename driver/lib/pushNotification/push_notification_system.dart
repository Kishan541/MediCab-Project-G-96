...
class PushNotificationSystem{
  ...
  Future initializeCloudMessaging(BuildContext context) async {
    //1. Terminated
    //When the app is closed and opened from the push notification
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage? remoteMessage) {
     ...
     // readUserRideRequestInformation(userRideRequest!.data["rideRequestId"], context);
    });
  ...
  readUserRideRequestInformation(String userRideRequestId, BuildContext context) {
    FirebaseDatabase.instance.ref().child("All Ride Requests").child(userRideRequestId).child("driverId").onValue.listen((event) {
      if(event.snapshot.value == "waiting" || event.snapshot.value == fAuth.currentUser!.uid){
        ...
        Fluttertoast.showToast(msg: "The Ride Request has been cancelled");
